...
function scheduleRetry(msg, retries) {
    ...
    node.debug(`Scheduling retry ${retries} in ${jitteredDelay.toFixed(0)}ms`);
    ...
    setTimeout(() => {
        node.log(`Executing retry ${retries} of ${config.retryLimit}. Sending message.`);
...
function resetRetryMechanism() {
    ...
    node.log('Retries count reset to 0.');
...
function startTimeout() {
    ...
    timeoutTimer = setTimeout(() => {
        node.error(`Operation timed out after ${config.timeoutDuration / 60000} minutes without new messages.`);
        resetRetryMechanism();
...
try {
    ...
    if (countdownActive) {
        if (msg.payload === config.successPayload) {
            ...
            resetRetryMechanism();
            node.log(`Reset message received: ${msg.payload}. Retry mechanism reset.`);
            ...
            node.warn(`Message dropped due to active countdown. Payload: ${msg.payload}`);
            node.done();
            return null;
        }
    }
    ...
    if (msg.payload === config.successPayload) {
        ...
        resetRetryMechanism();
        node.log(`Success: ${msg.payload}. Retry mechanism reset.`);
        node.status({ fill: 'green', shape: 'dot', text: 'Success: rearmed' });
        node.done();
        return null;
    } else {
        if (retries === 0) {
            ...
            node.debug(`Initial rearm attempt scheduled as retry ${retries}.`);
            ...
            retries += 1;
            context.set('retries', retries);
            node.debug(`Retry attempt ${retries} for payload: ${msg.payload}.`);
            scheduleRetry(msg, retries);
            // Do not send the message immediately; it will be sent by the scheduler
            return;
        } else {
            // Exceeded retry limit: consider operation failed
            node.error(`Rearm failed after ${config.retryLimit} attempts.`, msg);
            resetRetryMechanism();
